...
int main(int argc, char* argv[]) {
 ...
 //   if (strncmp(argv[i],"-selector",9)==0){
  ...
  if(DO_LIST){
	...
	std::string xrd("root://cmsxrootd.fnal.gov/");
	std::string xrdfile(MyRootFile);
	std::string fullfile = xrd+xrdfile;
	filenames.push_back(fullfile);
...
std::cout<<"creating tree template"<<std::endl;
  NanoTreeTemplate s(chain);
